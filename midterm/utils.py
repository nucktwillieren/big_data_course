...
def create_grids_iter(width, height, margin, number, offset_x=0, offset_y=0):
    y, x = get_multiply(number)
    width_of_content = width / x - margin*2
    width_of_blocks = width / x
    height_of_content = height / y - margin*2
    height_of_blocks = height / y
    dis_to_center_of_blocks_x, dis_to_center_of_blocks_y = width_of_blocks / \
        2, height_of_blocks / 2
    ...
    for i in range(y):
        ...
        for j in range(x):
            cursor_x += width_of_blocks
            yield cursor_x, cursor_y, width_of_content, height_of_content


def create_random_grids_iter(width, height, x, y, number, offset_x=0, offset_y=0):
    width_of_blocks = width / x
    height_of_blocks = height / y
    dis_to_center_of_blocks_x, dis_to_center_of_blocks_y = width_of_blocks / \
        2, height_of_blocks / 2
